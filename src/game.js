...
function drawTile(stage, posx, posy, color) {
	...
	stage.addChild(shape).set({
		x : posx,
		y : posy
	}).graphics.f(color).dc(0, 0, 50);
	return shape;
}
...
var init = function () {
	...
	var figure;
	var tile1 = new Tile(drawTile(stage, sixwidth, halfheight, "red"));
	var tile2 = drawTile(stage, sixwidth * 2, halfheight, "red");
	tile2.on("click", function (evt) {
		jump(stage, figure, dim, 2);
		tile1.disappear();
	});
	var tile3 = new Tile(drawTile(stage, sixwidth * 3, halfheight, "red"));
	tile3.on("click", function (evt) {
		jump(stage, figure, dim, 3);
		tile2.disappear();
	});
	var tile4 = new Tile(drawTile(stage, sixwidth * 4, halfheight, "red"));
	tile4.on("click", function (evt) {
